    ...
    async def async_update_data():
        ...
        async with async_timeout.timeout(options.get("data_collection_timeout_seconds", COLLECTION_TIMEOUT_SECONDS)):
            ...
            for description in PHASE_SENSORS:
                ...
                elif description.key.startswith("consumption_net_"):
                    data[description.key] = await envoy_reader.consumption_net_phase(
                        description.key
                    )
                ...
                elif description.key.startswith("lifetime_production_net_"):
                    data[
                        description.key
                    ] = await envoy_reader.lifetime_production_net_phase(description.key)
                elif description.key.startswith("lifetime_consumption_"):
                    data[
                        description.key
                    ] = await envoy_reader.lifetime_consumption_phase(description.key)
                elif description.key.startswith("lifetime_consumption_net_"):
                    data[
                        description.key
                    ] = await envoy_reader.lifetime_consumption_net_phase(description.key)
